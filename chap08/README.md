...
## 08-5. 웹 워커
웹 브라우저는 웹 페이지를 하나의 쓰레드로 처리하기 때문에, 시간이 오래 걸리는 작업을 처리하는 경우에는 다른 작업을 못하고 기다리거나 페이지가 멈추기도 합니다. 이런 문제를 해결하기 위해 등장한 것이 백그라운드에서 작업을 할 수 있게 하는 워커(worker)입니다. 워커 실행 후에는 웹 상에서 어떤 작업을 해도 워커에는 영향을 미치지 않습니다.
...
## 08-7. SVG 요소
SVG는 XML 기반의 파일 형식으로 W3C에서 개발한 오픈 표준의 벡터 그래픽 파일 형식입니다.

### SVG란
SVG(Scalable Vector Graphic) 파일은 영문 그대로 해석하면 확장 가능한 벡터 그래픽 파일입니다. XML을 기반으로 하고 있기 때문에 웹에서 그대로 볼 수 있고, SVG 기반의 편집 프로그램 뿐만 아니라 문서 편집기로도 편집이 가능합니다.

SVG는 CSS로 스타일을 지정할 수도 있고 SVG DOM을 이용하면 동적인 행동도 추가할 수 있습니다. 또한 `<object>`나 `<embed>` 태그를 이용해서 SVG 콘텐츠를 웹 문서에 삽입할 수도 있습니다.